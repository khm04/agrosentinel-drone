...
MODEL            = PRETRAINED_MODEL if PRETRAINED_MODEL else "yolov8n.pt"  # nano: ~6x lighter than yolov8m, fits 4GB GPUs
...
IMG_SIZE        = 640                   # 640 fits laptop GPUs (4-6GB); raise to 1280 only on a server-grade GPU
...
def count_train_images():
    ...
    try:
        with open(DATASET) as f:
            d = yaml.safe_load(f)
        base = Path(d.get("path", Path(DATASET).parent))
        train_rel = d.get("train", "train/images")
        train_dir = base / train_rel
        if not train_dir.is_absolute():
            train_dir = Path(DATASET).parent / train_rel  # fallback if path: missing
...
def auto_batch(vram_gb, gpu_count, dataset_size=None):
    ...
    else:               batch_per_gpu = 4   # 4GB laptop GPUs: yolov8n @ 640px fits batch=4 comfortably

    total_batch = batch_per_gpu * gpu_count

    # Avoid a dangling final batch of exactly 1 image (crashes BatchNorm).
